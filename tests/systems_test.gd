...
func _initialize() -> void:
	...
	# --- database ---
	assert(db.units.size() == 77, "expected 77 units, got %d" % db.units.size())
	...
	assert(db.playable_pool(4).size() == 21, "Wayfarer pool should be 21")
	assert(db.playable_pool(3).size() == 35, "Novice pool should be 35")
